...
(() => {
    ...
    const defs = [
        ...
        // 显示 / 语音
        {
            ...
            msg: "显示表情 %1",
            args: [{ type: "input_value", name: "EYE", check: "Number" }],
        },
        {
            type: "xiaobai_show_num",
            msg: "显示数字 %1",
            args: [{ type: "input_value", name: "NUM", check: "Number" }],
        },
        { type: "xiaobai_show_off", msg: "关闭显示", args: [] },
        {
            type: "xiaobai_play_voice",
            msg: "播放词条 P%1",
            args: [{ type: "input_value", name: "ITEM", check: "Number" }],
        },
        {
        ...
        // 控制
        {
            type: "xiaobai_wait",
            msg: "等待 %1 秒（电脑本地）",
            args: [{ type: "input_value", name: "SECONDS", check: "Number" }],
        },
        ...
        B[def.type] = {
            init() {
                this.appendDummyInput().appendField(
                    def.msg.replace(/%\d/g, "").trim(),
                );
                for (const arg of def.args) {
                    if (arg.type === "field_dropdown") {
                        this.appendDummyInput().appendField(
                            new Blockly.FieldDropdown(arg.options),
                            arg.name,
                        );
                    } else if (arg.type === "input_value") {
                        this.appendValueInput(arg.name).setCheck(
                            arg.check || null,
                        );
                    }
                }
