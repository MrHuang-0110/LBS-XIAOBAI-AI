...
| 直接控制 | 单电机正反转/停、组合移动、功率档、关闭显示、**停止程序**（始终可用） |
| Blockly 编程 | 拖积木 → 运行/停止；切到本页会自动发 `ENTER_PROGRAM`（设备四灯跑马、播 ID 52） |
| 状态 | 每 500ms 查询：模式、三路红外、电量、当前任务、故障位 |
| 协议日志 | 每条 C2/D2/D3 的收发记录，排障用 |

- 执行器在等待 DONE 期间持续发 300ms 心跳；设备丢首包时会 200ms 重发，不会卡流程。
...
.venv\Scripts\python.exe -m pytest        # 36 项：协议黄金帧/AST/执行器/工程读写
```
...
src/xiaobai/ui/            PySide6 界面 + QWebChannel 桥
