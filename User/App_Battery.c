...
void App_Battery_Update(void)
{
    ...
    /* D3 LOW_BATTERY 事件：只在低电量状态翻转时上报一次（0xFF=首轮建基线） */
    static uint8_t last_low = 0xFF;
    if (last_low == 0xFF) {
        last_low = low;
        ...
        uint8_t d[8] = {0};
        d[0] = low;
        d[1] = (uint8_t)(mv & 0xFFU);
        d[2] = (uint8_t)(mv >> 8);
        Proto_Ble_SendEvent(PROTO_EVT_LOW_BATTERY, d);
    }
}
